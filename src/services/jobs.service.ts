...
export async function getJobs(): Promise<ApiResult<Job[]>> {
  try {
    const data = await httpGet<unknown>('/job/execution?comprehensivePage=true')

    // Validate response with Zod
    const parsed = JobExecutionListSchema.safeParse(data)
    ...
    // Transform to normalized Job format
