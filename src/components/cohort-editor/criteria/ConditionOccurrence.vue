...
const attributeSpecs = computed<CriteriaAttributeSpec[]>(() => [
  ...
  },
  {
    ...
    kind: 'textFilter',
    componentProps: () => createSchemaFieldProps(
      ensureObjectField(conditionOccurrenceData.value, 'StopReason', () => ({ Text: '', Op: 'contains' })) as TextFilter
    ),
    init: () => {
      ensureObjectField(conditionOccurrenceData.value, 'StopReason', () => ({ Text: '', Op: 'contains' }))
    },
    clear: () => {
