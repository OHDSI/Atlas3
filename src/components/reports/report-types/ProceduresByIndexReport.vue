...
<template>
  <v-card elevation="0">
    ...
    <v-card-text>
      ...
      >
        No procedures by index data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'procedures-by-index')
  ...
  if (currentReport.value && 'prevalence' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'conceptName', title: 'Procedure Name', sortable: true },
