...
<template>
  <v-card elevation="0">
    <v-card-title class="text-h6">
      ...
      Persons Exposure - Baseline Period
    </v-card-title>
    ...
    <v-card-text>
      ...
      >
        No baseline period persons/exposure data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'persons-exposure-baseline')
  ...
  if (currentReport.value && 'prevalence' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'conceptName', title: 'Concept Name', sortable: true },
