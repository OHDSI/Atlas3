...
<template>
  <v-card elevation="0">
    <v-card-title class="text-h6">
      ...
      Drug Utilization - Baseline Period
    </v-card-title>
    ...
    <v-card-text>
      ...
      >
        No baseline drug utilization data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'drug-utilization-baseline')
  ...
  if (currentReport.value && 'prevalence' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'conceptName', title: 'Drug Name', sortable: true },
