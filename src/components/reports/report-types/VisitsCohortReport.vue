<!--
  VisitsCohortReport Component
  Feature: 005-cohort-reports
  Task: T095

  Displays cohort period visits data in a table format
-->
<template>
  <v-card elevation="0">
    <v-card-title class="text-h6">
      <v-icon class="mr-2">mdi-hospital-building</v-icon>
      Visits - Cohort Period
    </v-card-title>
    ...
    <v-card-text>
      ...
      >
        No cohort period visits data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'visits-cohort')
