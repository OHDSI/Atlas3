...
<template>
  <v-card elevation="0">
    ...
    <v-card-text>
      ...
      >
        No conditions by index data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'conditions-by-index')
  ...
  if (currentReport.value && 'prevalence' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'conceptName', title: 'Condition Name', sortable: true },
