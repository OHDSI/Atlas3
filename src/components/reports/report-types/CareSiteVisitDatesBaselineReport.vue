...
<template>
  <v-card elevation="0">
    <v-card-title class="text-h6">
      ...
      Care Site Visit Dates - Baseline Period
    </v-card-title>
    ...
    <v-card-text>
      ...
      >
        No baseline care site visit dates data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'care-site-visit-dates-baseline')
  ...
  if (currentReport.value && 'data' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'careSiteName', title: 'Care Site Name', sortable: true },
