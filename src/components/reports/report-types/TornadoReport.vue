...
<template>
  <v-card elevation="0">
    <v-card-title class="text-h6">
      <v-icon class="mr-2">mdi-chart-waterfall</v-icon>
      Tornado Diagram
    </v-card-title>
    ...
    <v-card-text>
      ...
      >
        No tornado diagram data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'tornado')
  ...
  if (currentReport.value && 'prevalence' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'conceptName', title: 'Parameter Name', sortable: true },
        ...
        { key: 'prevalence', title: 'Impact (%)', sortable: true, align: 'end' }
      ],
      rows: data.map(item => ({
