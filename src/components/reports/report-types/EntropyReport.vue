...
<template>
  <v-card elevation="0">
    ...
    <v-card-text>
      ...
      >
        No entropy analysis data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'entropy')
  ...
  if (currentReport.value && 'prevalence' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'conceptName', title: 'Metric Name', sortable: true },
        ...
        { key: 'prevalence', title: 'Entropy (%)', sortable: true, align: 'end' }
      ],
      rows: data.map(item => ({
