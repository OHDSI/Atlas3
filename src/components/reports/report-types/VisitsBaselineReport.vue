<!--
  VisitsBaselineReport Component
  Feature: 005-cohort-reports
  Task: T092

  Displays baseline period visits data in a table format
-->
<template>
  <v-card elevation="0">
    <v-card-title class="text-h6">
      <v-icon class="mr-2">mdi-hospital-building</v-icon>
      Visits - Baseline Period
    </v-card-title>
    ...
    <v-card-text>
      ...
      >
        No baseline period visits data available
      </v-alert>
    </v-card-text>
  </v-card>
</template>
...
async function loadData() {
  ...
  await loadReport(props.cohortId, props.sourceKey, 'visits-baseline')
  ...
  if (currentReport.value && 'prevalence' in currentReport.value) {
    ...
    tableData.value = {
      headers: [
        ...
        { key: 'conceptName', title: 'Visit Type', sortable: true },
