<template>
  <v-card class="pathway-card" @click="emit('open', pathway.id ?? 0)">
    <v-card-title>{{ pathway.name }}</v-card-title>
    <v-card-subtitle v-if="pathway.description">{{ pathway.description }}</v-card-subtitle>
    <v-card-text>
      <div class="meta">
      ...
      <div class="tags">
        <v-chip v-for="t in pathway.tags" :key="t.id" size="x-small">{{ t.name }}</v-chip>
      </div>
    </v-card-text>
    <v-card-actions>
      <v-spacer />
