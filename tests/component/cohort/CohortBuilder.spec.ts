...
describe('CohortBuilder', () => {
  ...
  const inclusionCriteriaPanel = (wrapper: Wrapper) =>
    wrapper.findComponent({ name: 'CohortExpressionEditor' })
  ...
  it('buildCohortExpression keeps a concept set whose id is 0 (single concept-set cohort)', async () => {
    ...
    const exported = (wrapper.vm as any).buildExportCohort()
    expect(exported.conceptSets).toHaveLength(1)
    expect(exported.conceptSets[0].id).toBe(0)
  })
  ...
  it('handleSave sends every field the editor holds, not just some of them', async () => {
    ...
    const definition = vi.mocked(cohortService.saveCohortDefinition).mock.calls[0]?.[0] as unknown as {
  ...
  // Regression for a Critical: version preview must signal the mounted editor
  // to reload the historical version instead of mutating the current cohort
  // with the raw DTO shape directly.
  it('entering version preview updates preview state and emits a reload signal', async () => {
    const wrapper = createWrapper({ id: '42' })
    await new Promise(r => setTimeout(r, 0))
    await wrapper.vm.$nextTick()
    ...
    expect(store.previewVersion?.version).toBe(3)
    expect((store as unknown as { reloadVersion: number | null }).reloadVersion).toBe(3)
  })
