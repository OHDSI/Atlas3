 ...
 * VersionsTabContent.vue's handlePreview (ASSET_ROUTE_SEGMENT) and
 * handleCopy (ASSET_DETAIL_ROUTE_SEGMENT) build and asserts they land on
 * the intended route rather than falling through to 'not-found'.
 */
import { describe, it, expect } from 'vitest'
import { createMemoryHistory, createRouter, type RouteRecordRaw } from 'vue-router'
import { defineComponent, h } from 'vue'
import { routes } from '@/router/routes'
import { ASSET_ROUTE_SEGMENT, ASSET_DETAIL_ROUTE_SEGMENT } from '@/components/versions/types'
...
const expectedDetailRoute: Partial<Record<VersionsConfig['assetType'], string>> = {
  cohortdefinition: 'cohort-edit',
  'pathway-analysis': 'pathway-edit',
  ir: 'incidence-rate-edit',
}

describe('post-copy navigation targets resolve to real routes', () => {
  const router = makeRouter()

  for (const assetType of Object.keys(ASSET_DETAIL_ROUTE_SEGMENT) as VersionsConfig['assetType'][]) {
    it(`${assetType} -> /${ASSET_DETAIL_ROUTE_SEGMENT[assetType]}/:id resolves`, () => {
      const path = `/${ASSET_DETAIL_ROUTE_SEGMENT[assetType]}/5`
      const resolved = router.resolve(path)

      expect(resolved.name).not.toBe('not-found')
      expect(resolved.name).toBe(expectedDetailRoute[assetType])
    })
  }

  it('has no conceptset entry - concept sets have no id-addressable route to navigate to', () => {
    expect('conceptset' in ASSET_DETAIL_ROUTE_SEGMENT).toBe(false)
  })

  it('the pre-fix "/pathway-analysis/:id" copy target would have been a dead link', () => {
    const resolved = router.resolve('/pathway-analysis/5')
    expect(resolved.name).toBe('not-found')
  })
})
