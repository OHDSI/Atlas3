/**
 * Unit Tests: Characterization Versions Service
 *
 * Tests for cohort-characterization version history API calls.
 * Mirrors the cohort-definition-versions.service tests.
 */
...
import {
  ...
  updateVersion,
  copyVersion,
} from '@/services/characterization-versions.service'
...
describe('CharacterizationVersionsService', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  describe('getVersions', () => {
    it('returns versions for a characterization', async () => {
      ...
      expect(httpGet).toHaveBeenCalledWith('/cohort-characterization/123/version/')
  ...
  describe('getVersion', () => {
    it('returns a specific version', async () => {
      ...
      expect(httpGet).toHaveBeenCalledWith('/cohort-characterization/123/version/2')
  ...
  describe('updateVersion', () => {
    it('updates version comment and archived status', async () => {
      ...
      expect(httpPut).toHaveBeenCalledWith(
        '/cohort-characterization/123/version/2',
        { comment: 'Updated', archived: true }
      )
    ...
    it('throws on validation error for response', async () => {
      ...
      await expect(
        updateVersion(123, 2, { comment: 'Test', archived: false })
      ).rejects.toThrow('Failed to validate updated version data')
    })
  ...
  describe('copyVersion', () => {
    it('creates a new characterization from a version', async () => {
      const { httpPut } = await import('@/services/http-client')
      const newDesign = {
        id: 456,
        name: 'Copy of Test Characterization',
        cohorts: [],
        featureAnalyses: [],
        stratas: [],
      }

      vi.mocked(httpPut).mockResolvedValue(newDesign)

      const result = await copyVersion(123, 2)

      expect(httpPut).toHaveBeenCalledWith(
        '/cohort-characterization/123/version/2/createAsset'
      )
